...
L_spat=40
L_time=40
output_path_electric=./result/flux_tube_electric
output_path_magnetic=./result/flux_tube_magnetic
T_min=4
T_max=20
R_min=4
R_max=20
parameters="-conf_format_plaket ${conf_format_plaket} -conf_path_plaket ${conf_path_plaket}\
    ...
    -output_path_electric ${output_path_electric} -output_path_magnetic ${output_path_magnetic}\
    -L_spat ${L_spat} -L_time ${L_time}\
    -T_min ${T_min} -T_max ${T_max} -R_min ${R_min} -R_max ${R_max}"

../flux_tube_long_${matrix_type_plaket}_${matrix_type_wilson}_test ${parameters}