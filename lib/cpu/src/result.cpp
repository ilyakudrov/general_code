...
result::result() { array = std::vector<double>(0); }
void result::average(double a[2]) {
  ...
  for (int i = 0; i < array.size(); i++) {
    err += (array[i] - aver) * (array[i] - aver) / array.size() /
           (array.size() - 1);
  }
  err = powf(err, 0.5);
  a[0] = aver;
...
void result::read(char const *file_name, int size1) {
  ...
  if (!stream.read((char *)&array[0], size1 * sizeof(double)))
...
void result::get_hist(int n, result &plaket, result &number) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    for (int j = 0; j < array.size(); j++) {
      if (array[j] >= (min + i * step) && array[j] <= (min + (i + 1) * step))
        count += 1;
    }
...
void average_jack_wilson(double a[2], result &val1, result &val2,
                         result &val3) {
  int size = val1.array.size();
  std::vector<double> vec(size);
  double aver1[2];
...
void average_jack_sum(double a[2], result &val11, result &val12, result &val2,
  ...
  for (int i = 0; i < size; i++) {
    vec[i] = (val11.average_n(i) + val12.average_n(i)) / val2.average_n(i) -
             val3.average_n(i) - val4.average_n(i);
  }
  double sigma = 0;
  for (int i = 0; i < size; i++) {
    sigma += (vec[i] - b) * (vec[i] - b);
