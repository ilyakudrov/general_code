...
void dirac_mult(scomplex_t *res, const scomplex_t *src, int place,
  ...
  for (int mu = 1; mu <= 4; mu++) {
    ...
    link_ferm.move(mu, 1);
    matrix_mult_complex1(
        exp(mu_q * delta_4) / 2 * sign * link.get_matrix1(data_conf),
        &src[complex_place(link_ferm)], &vec, src_index, border_sign);
...
/*void dirac_mult_conj(scomplex_t* res, const scomplex_t* src, int place,
   matrix* data_conf, int x_size, int y_size, int z_size, int t_size){ int t =
   place/(x_size * y_size * z_size*2) + 1; int z = (place - (x_size * y_size *
   z_size*2)*(t - 1))/(x_size * y_size*2) + 1; int y = (place - (x_size * y_size
   * z_size*2)*(t - 1) - (2*x_size * y_size)*(z - 1))/(x_size*2) + 1; int x =
   (place - (x_size * y_size * z_size*2)*(t - 1) - (2*x_size * y_size)*(z - 1) -
   2*x_size*(y - 1))/2 + 1; int src_index = place - ((t - 1) * 2 * x_size *
   y_size * z_size
                + (z - 1) * 2 * x_size * y_size
                + (y - 1) * 2 * x_size
                + (x - 1) * 2);
...
int complex_place(link1<matrix> &link) {
  return (link.coordinate[3]) * 2 * link.lattice_size[0] *
             link.lattice_size[1] * link.lattice_size[2] +
         (link.coordinate[2]) * 2 * link.lattice_size[0] *
             link.lattice_size[1] +
...
void matrix_mult_complex1(matrix A, const scomplex_t *a, scomplex_t *a1, int i,
  ...
  a1->re = border_sign * (a[0].re * z1.re - a[0].im * z1.im + a[1].re * z2.re -
                          a[1].im * z2.im);
  a1->im = border_sign * (a[0].re * z1.im + a[0].im * z1.re + a[1].re * z2.im +
                          a[1].im * z2.re);
}
...
void test_eigenvector(const scomplex_t *eigenvector, scomplex_t eigenvalue,
                      ...
                      int z_size, int t_size, FLOAT mass, FLOAT mu_q,
                      FLOAT tolerance) {
  scomplex_t *vec;
  ...
  for (int i = 0; i < size; i++) {
    ...
    dirac_mult(&res, eigenvector, i, data_conf, x_size, y_size, z_size, t_size,
               mass, mu_q);
    vec[i].re = res.re;
    vec[i].im = res.im;
  }
  for (int i = 0; i < size; i++) {
    vec[i].re -=
        eigenvector[i].re * eigenvalue.re - eigenvector[i].im * eigenvalue.im;
    vec[i].im -=
        eigenvector[i].re * eigenvalue.im + eigenvector[i].im * eigenvalue.re;
  }
  if (test_module(vec, size) < tolerance)
    printf("eigenvector is right \n");
  else
    printf("eigenvector is not right \n");
  free(vec);
}
