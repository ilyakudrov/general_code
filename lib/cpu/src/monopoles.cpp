...
loop::loop(const link1<matrix> n1) {
  node = link1<matrix>(n1);
  for (int i = 0; i <= 6; i++)
    link[i] = NULL;
};
loop::loop() {
...
int test(link1<matrix> l, FLOAT *J) {
  for (int i = 1; i <= 4; i++) {
    if ((J[l.get_place1() * 4 + i - 1] > 0.3) ||
        (J[l.get_place1() * 4 + i - 1] < -0.3))
      return i;
  }
  for (int i = 1; i <= 4; i++) {
    l.move(i, -1);
...
void find_cluster(loop *ll, FLOAT *J) {
  ...
  do {
    ...
    tmp = ll->node;
    if (dir > 0) {
      J[(ll->node.get_place1() * 4) + dir - 1] = 0.;
      tmp.move(dir, 1);
    };
    if (dir < 0) {
      link1<matrix> tmp1(ll->node);
      tmp1.move(-dir, -1);
      J[(tmp1.get_place1() * 4) - (dir)-1] = 0.;
      tmp.move(-dir, -1);
    };
