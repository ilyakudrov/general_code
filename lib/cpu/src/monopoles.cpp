...
loop::loop (const link1 n1) {node=link1(n1); for(int i=0; i<=6; i++) link[i]=NULL;};
loop::loop() { node=link1(); for(int i=0; i<=6; i++) link[i]=NULL; };
loop::loop( const loop& l) { node=link1(l.node);  for(int i=0; i<=6; i++) link[i]=l.link[i]; };
...
void find_cluster(loop* ll, double* J) {
  ...
  do {
    ...
    if (dir<0)  {link1 tmp1(ll->node); tmp1.move(-dir,-1); 
    ...
    for(int iz=1;iz<=z_size; iz++ ) {
      for(int iy=1;iy<=y_size; iy++ ) {
        for(int ix=1;ix<=x_size; ix++ ) {
          link1 tmp(ix,iy,iz,it, x_size, y_size, z_size, t_size);
          dir1=test(tmp, J);
          if(dir1==0) continue;
