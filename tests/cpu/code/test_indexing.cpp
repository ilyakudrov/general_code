...
int main(int argc, char *argv[]) {
  ...
  start_time = omp_get_wtime();
  wilson_loops = wilson_parallel_indexed_single_rxt(conf2.array, R_min, R_max,
                                                    T_min, T_max);
  end_time = omp_get_wtime();
  search_time = end_time - start_time;
  ...
  start_time = omp_get_wtime();
  wilson_loops = wilson_parallel_indexed_single_rxt(conf3.array, R_min, R_max,
                                                    T_min, T_max);
  end_time = omp_get_wtime();
  search_time = end_time - start_time;
  std::cout << "wilson loop parallel indexed single_rxt su3_eigen time: "
            << search_time << std::endl;
  std::cout << "wilson_loops:" << std::endl;
  for (auto it = wilson_loops.begin(); it != wilson_loops.end(); it++) {
    cout << get<0>(it->first) << "," << get<1>(it->first) << "," << it->second
