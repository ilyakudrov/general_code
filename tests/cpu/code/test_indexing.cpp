...
int main(int argc, char *argv[]) {
  ...
  Data::LatticeData<DataPatternSeparateDir, MATRIX_TYPE> data_indexed(lat_dim);
  Data::read_data_convert(data_indexed, conf_path1, "qcdstag", 0, "double", 1);

  start_time = omp_get_wtime();
  std::cout << polyakov_loop(data_indexed) << std::endl;
  end_time = omp_get_wtime();
  search_time = end_time - start_time;
  ...
  Data::LatticeData<DataPatternLexicographical, MATRIX_TYPE> data_indexed1(
      lat_dim);
  Data::read_data_convert(data_indexed1, "../../confs/test/conf_test",
                          "lexicographical", 0, "double", 0);

  start_time = omp_get_wtime();
  std::cout << polyakov_loop(data_indexed1) << std::endl;
  end_time = omp_get_wtime();
  search_time = end_time - start_time;
